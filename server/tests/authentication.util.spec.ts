...
export class AuthenticationTestUtility {
    ...
    public static async cleanupIdentityApi() {
        ...
        systemAuthToken = await AuthenticationUtil.getSystemUserToken();
    ...
    public static async createIdentityApiTestData(): Promise<any> {
        ...
        const adminUserRes = await IdentityApiService.registerUser(CONST.testing.PRODUCT_ADMIN_EMAIL);
        const editorUserRes = await IdentityApiService.registerUser(CONST.testing.PRODUCT_EDITOR_EMAIL);
        ...
        // Now we grant the user the role we need.
